    ...
    try:
        ...
        for file in os.listdir(eu.files_dir):
            if file.endswith(".ndjson"):
                ...
                files = {
                    "file": (file, open(os.path.join(eu.files_dir, file), "rb")),
                }
                response = requests.post(
                    api_ip, headers=headers, files=files, auth=(username, password)
                )
...
def import_dashboard(dashboard_filename):
    ...
    try:
        ...
        api_ip = (
            "http://" + meas_node_ip + "/api/saved_objects/_import?createNewCopies=true"
        )
        headers = {
            "kbn-xsrf": "true",
        }
        files = {
            "file": (
                dashboard_filename,
                open(os.path.join(eu.dashboards_dir, dashboard_filename), "rb"),
            ),
        }
        response = requests.post(
            api_ip, headers=headers, files=files, auth=(username, password)
        ...
        if response.status_code == 200:
            ...
            ret_val[
                "msg"
            ] += f"Failed to import dashboard {dashboard_filename} to kibana. HTTP Error {response.status_code}. "
