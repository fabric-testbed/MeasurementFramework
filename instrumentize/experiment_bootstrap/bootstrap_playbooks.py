...
def main():
    ...
    play_recap_pip_docker = decoded_out_pip_docker[
        decoded_out_pip_docker.find("PLAY RECAP") :
    ]
    ...
    if r_pip_docker.returncode == 0:
        ...
        ret_val["pip3_docker_sdk"][
            "msg"
        ] = f"Pip & Docker SDK playbook install failed..{decoded_err_pip_docker}"
    ...
    ret_val["docker"] = {}
    playbook = (
        "/home/mfuser/mf_git/instrumentize/experiment_bootstrap/docker_playbook.yml"
    )
    ...
    ret_val["success"] = (
        (r_pip_docker.returncode == 0)
        and (r_docker.returncode == 0)
        and (r_ptp.returncode == 0)
    )
